...
| Output entropy over a small model's next-token distribution can be measured | Established | The entropy pilot was run (see Appendix E). The *measurement* is real; its interpretation is not yet load-bearing. |
...
| "Specificity-to-reality" / confidence–reality divergence as a usable detection signal | Exploratory | The entropy pilot is only the output-side shadow; the correctness side needs ground truth, not yet operationalized at scale. |
...
| S = 1 − e^(−TC) as a metric of experiential saturation / selfhood | Speculative (kept separate) | Speculative geometry. Earns "might be modeled as," never "is." S over attention-correlation matrices is pending; only its output-side shadow has been touched. |
