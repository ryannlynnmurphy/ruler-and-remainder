...
const ITEM_KIND = new Set(["note", "essay", "draft", "transcript", "report", "prompt", "output", "reflection", "source"]);
...
export async function saveCorpusItem({ title, kind, authorship, status, project, source, body, analysis = {} } = {}) {
  const sql = db();
  if (!sql) return null;
  ...
  let vectors = null;
  try {
    if (await ensureVectorColumn()) vectors = await embed(chunks.map((c) => c.text), "document");
...
export function ensureVectorColumn() {
  ...
  _vectorReady = (async () => {
    ...
    await sql.unsafe(`ALTER TABLE reading_chunks ADD COLUMN IF NOT EXISTS embedding vector(${EMBED_DIM})`);
    await sql`
